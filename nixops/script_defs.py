...
def op_info(args):
    ...
    if args.all:
        ...
        if args.plain:
            ...
            if depl.nix_path != []: print("Nix path:", " ".join(["-I " + x for x in depl.nix_path]))
            if depl.rollback_enabled: print("Nix profile:", depl.get_profile())
            if depl.args != {}: print("Nix arguments:", ", ".join([n + " = " + v for n, v in depl.args.items()]))
            print()
...
def op_ssh_for_each(args):
    ...
    for depl in one_or_all(args):
      ...
      results = results + nixops.parallel.run_tasks(
          ...
          tasks=iter(depl.active.values()), worker_fun=worker)
...
def setup_logging(args):
    if os.path.exists('/dev/log') \
        ...
        # determine user
        try:
            user = subprocess.check_output(['logname'],stderr=subprocess.PIPE).strip().decode()
        except:
